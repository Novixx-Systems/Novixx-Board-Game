...
from fairy.drawback import check_drawback
import chess
try:
...
STANDARD_FEN = "uz7PU/jz7PJ/rz7PR/zz7PP/lzz5PPL/sz7PS/rz7PR/jz7PJ/uz7PU w - - 0 1"
...
class FairyBoard:
    ...
    def push(self, move, append=True, drawback="atheist"):
        try:
            # log.debug("move=%s, fen=%s", move, self.fen
            # check if the move is against the drawback
            if append:
    ...
    def square_to_index(self, square: str):
        col = ord(square[0]) - ord("a")
        row = 8 - int(square[1])
        return row * 8 + col

    def piece_at(self, square: int, toUpper: bool = True): # square is 0-63
        # expand fen to 8x8 board (replace numbers with dots)
        # if square is string, convert to int (e.g. "a1" -> 0)
        if isinstance(square, str):
            square = self.square_to_index(square)
        ...
        board = board.replace("8", "........") # too lazy to create a loop haha
        return board[square].upper() if board[square].isalpha() and toUpper else board[square]

    def get_san(self, move):
        return self.sf.get_san(self.variant, self.fen, move, self.chess960, self.notation)

    def has_legal_move(self, drawback="atheist"):
    ...
    def is_capture(self, move):
        dest = move[2:4]
        return self.piece_at(dest).isalpha()

    def can_capture(self):
        board = FairyBoard("chess", self.fen)
        moves = board.legal_moves(None)
        if len(moves) == 0:
            return False
        else:
            any_is_capture = False
            for move in moves:
                if board.is_capture(move):
                    any_is_capture = True
                    break
            return any_is_capture

    def legal_moves(self, drawback="atheist"):
