...
from fairy.drawback import DRAWBACK_DESCRIPTION
...
def add_game_context(game, ply, user, context):
    shownWD = game.whiteDrawback if user == game.wplayer else (game.whiteDrawback if game.result != "*" else None)
    shownBD = game.blackDrawback if user == game.bplayer else (game.blackDrawback if game.result != "*" else None)
    # if Fairy-Stockfish vs Fairy-Stockfish, show the drawback of the player who is not the user
    if game.wplayer.username == "Fairy-Stockfish" and game.bplayer.username == "Fairy-Stockfish":
        shownWD = game.whiteDrawback
        shownBD = game.blackDrawback
    context["gameid"] = game.id
    context["variant"] = game.variant
    context["wplayer"] = game.wplayer.username
    context["wtitle"] = game.wplayer.title
    # if complete show drawback of both players, if not show drawback of the player who is not the user
    context["whiteDrawback"] = shownWD
    context["blackDrawback"] = shownBD
    context["whiteDrawbackDescription"] = DRAWBACK_DESCRIPTION[shownWD] if shownWD else None
    context["blackDrawbackDescription"] = DRAWBACK_DESCRIPTION[shownBD] if shownBD else None
