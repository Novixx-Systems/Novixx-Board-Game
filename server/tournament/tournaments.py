...
async def get_latest_tournaments(app_state: PychessGlobalAppState, lang):
    ...
    async for doc in cursor:
        ...
        if tournament.frequency:
            try:
                tournament.translated_name = "NOVIXXIA TOURNAMENT" if tournament.variant == "chess" else "NOVIXXIA960 TOURNAMENT" if tournament.variant == "novixxia960" else "UNKNOWN TOURNAMENT NAME"
            except KeyError:
...
def translated_tournament_name(variant, frequency, system, lang_translation):
    ...
    return "NOVIXXIA TOURNAMENT" if variant == "chess" else "NOVIXXIA960 TOURNAMENT" if variant == "novixxia960" else "UNKNOWN TOURNAMENT NAME"