...
async def load_game(app_state: PychessGlobalAppState, game_id):
    ...
    game.whiteDrawback = doc.get("wd")
    game.blackDrawback = doc.get("bd")
...
async def import_game(request):
    ...
    document = {
        ...
        "by": data["username"],
        "wd": new_game.whiteDrawback,
        "bd": new_game.blackDrawback,
    }
...
async def join_seek(app_state: PychessGlobalAppState, user, seek, game_id=None, join_as="any"):
    ...
    if join_as == "player1":
        ...
        if seek.player1 is None:
            ...
            seek.player2 = user
        elif seek.player1 != seek.player2:
            return {"type": "seek_occupied", "seekID": seek.id}
...
async def new_game(app_state: PychessGlobalAppState, seek, game_id=None):
    ...
    if bplayer == wplayer:
        bplayer = seek.player2
        wplayer = seek.player1
...
async def insert_game_to_db(game, app_state: PychessGlobalAppState):
    ...
    document = {
        ...
        "z": int(game.chess960),
        "c": game.corr,
        "wd": game.whiteDrawback,
        "bd": game.blackDrawback,
    }
