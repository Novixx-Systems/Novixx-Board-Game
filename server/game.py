...
class Game:
    def __init__(
        ...
        self.has_legal_move = len(self.board.legal_moves(drawback=self.whiteDrawback if self.board.color == WHITE else self.blackDrawback)) > 0
    ...
    async def play_move(self, move, clocks=None, ply=None):
        ...
        self.legal_moves = self.board.legal_moves(drawback=self.whiteDrawback if self.board.color == WHITE else self.blackDrawback)
        log.info("legal_moves: %s", self.legal_moves, " drawback: %s", self.whiteDrawback if self.board.color == WHITE else self.blackDrawback)
        ...
        if self.status <= STARTED:
            try:
                ...
                try:
                    ...
                    if result == "WIN":
                        self.update_status(VARIANTEND, "0-1" if self.board.color == WHITE else "1-0")
                        await self.save_game()
                        if self.corr:
                            await opp_player.notify_game_end(self)
                        if self.simulId is not None:
                        ...
                        self.update_status(VARIANTEND, "1-0" if self.board.color == WHITE else "0-1")
                        await self.save_game()
                        if self.corr:
                            await opp_player.notify_game_end(self)
                        if self.simulId is not None:
                    ...
                    if self.wplayer.username == "Random-Mover" or self.bplayer.username == "Random-Mover":
                        self.update_status(INVALIDMOVE, "0-1" if self.board.color == WHITE else "1-0")
                        await self.save_game()
                        if self.corr:
                            await opp_player.notify_game_end(self)
                        if self.simulId is not None:
                ...
                if self.board.fen == oldfen:
                    self.board.fen = oldfen
                    raise SystemError("Invalid move %s" % move)
