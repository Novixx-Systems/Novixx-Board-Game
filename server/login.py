...
from seek import Seek, challenge
...
async def oauth(request):
    ...
    # set temporary cookie in our site with random value
    session["random_value"] = secrets.token_hex(16)
    random_value = session["random_value"]
    oauth_authorize_url = oauth_authorize_url.replace("BIDHERE", random_value)
    oauth_token_url = oauth_token_url.replace("BIDHERE", random_value)
    oauth_token_url = oauth_token_url.replace("BUSINESSTOKENHERE", random_value)
    ...
    if code is None:
        ...
        ) if provider != "novixx" else oauth_authorize_url.replace("BIDHERE", random_value)
...
async def login(request):
    ...
    if "token" not in session and "random_value" not in session:
        return web.HTTPFound(redirect_path)

    config = oauth_config.get(provider, oauth_config["lichess"])
    if provider == "novixx":
        account_api_url = config["account_api_url"].replace("BIDHERE", session["random_value"])
    else:
        account_api_url = config["account_api_url"]
    try:
        user_data = await get_user_data(account_api_url, session["token"] if "token" in session else "")
    except Exception as e:
        log.error("Failed to get user data for provider '%s': %s", provider, e)
        if provider == "novixx":
            del session["random_value"]
        return web.HTTPFound("/login/%s?error=failed_to_get_user_data" % provider)

    if "token" in session:
        del session["token"]
    if "random_value" in session:
        del session["random_value"]

    _id = user_data.get("id", "")
    if _id == "":
        _id = user_data.get("PERMISSION_USERNAME", "")
...
async def get_user_data(url, token):
    async with aiohttp.ClientSession() as client_session:
        ...
        async with client_session.get(url + "&PERMISSION_USERNAME=true", headers=data) as resp:
            data = await resp.text()
            # convert to json
            data = json.loads(data)
            # print("USER_DATA", data)
            # Novixx returns PERMISSION_USERNAME in the response, and it contains the username
            data["username"] = data.get("PERMISSION_USERNAME", "")
            return data
