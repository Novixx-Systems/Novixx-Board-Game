...
async def BOT_task(bot, app_state: PychessGlobalAppState):
    ...
    def AI_move(game, level):
        v = game.variant
        # if current player's color's drawback is "halfpawn", variant = halfpawn
        if game.whiteDrawback == "halfpawn" and game.board.color == WHITE:
            v = "halfpawn"
        elif game.blackDrawback == "halfpawn" and game.board.color == BLACK:
            v = "halfpawn"
        elif game.whiteDrawback == "queenhate" and game.board.color == WHITE:
            v = "noqueenwhite"
        elif game.blackDrawback == "queenhate" and game.board.color == BLACK:
            v = "noqueenblack"
        elif game.whiteDrawback == "kingofthehill" and game.board.color == WHITE:
            v = "kingofthehillblack" # inverted because white loses when black's king is on the center
        elif game.blackDrawback == "kingofthehill" and game.board.color == BLACK:
            v = "kingofthehillwhite" # inverted because black loses when white's king is on the center
